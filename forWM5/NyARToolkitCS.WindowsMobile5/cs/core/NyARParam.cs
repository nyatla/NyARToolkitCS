...
namespace jp.nyatla.nyartoolkit.cs.core
{
    ...
    public class NyARParam{
        ...
        {
            try {
                StreamReader s = new StreamReader(i_filename);
                loadFromARFile(s.BaseStream);
            } catch (Exception e) {
                throw new NyARException(e);
            }
        }
        public void loadFromARFile(Stream i_stream)
        {
            try {
        ...
        public int paramDisp()
        {
    	    ...
    	    Console.WriteLine("SIZE = "+xsize+", "+ysize);//printf("SIZE = %d, %d\n", param->xsize, param->ysize);
    	    Console.WriteLine("Distortion factor = "+dist_factor[0]+" "+dist_factor[1]+" "+dist_factor[2]+" "+dist_factor[3]);//printf("Distortion factor = %f %f %f %f\n", param->dist_factor[0],param->dist_factor[1], param->dist_factor[2], param->dist_factor[3] );
    	    for(int j = 0; j < 3; j++ ) {//for(j = 0; j < 3; j++ ) {
    	        for(int i = 0; i < 4; i++ ){
    	    	    Console.Write(array34[j*4+i]+" ");//printf("%7.5f ", param->mat[j][i]);
    	        }
    	        Console.WriteLine();//    printf("\n");
    	    }//}
            Console.WriteLine("--------------------------------------");//printf("--------------------------------------\n");
    ...
    //    }
         ...
         */
        private static NyARParam[] arParamLoad(Stream i_stream, int i_num)
        {
            try{
                int read_size=SIZE_OF_PARAM_SET*i_num;
                byte[] buf=new byte[read_size];
                BinaryReader br=new BinaryReader(i_stream);
...
//                bb.order(ByteOrder.BIG_ENDIAN);
                ...
                for(int i=0;i<i_num;i++){
                    ...
                    new_param.xsize = endianConv(br.ReadInt32());
                    new_param.ysize = endianConv(br.ReadInt32());
                    for(int i2=0;i2<3;i2++){
                        for(int i3=0;i3<4;i3++){
                            new_param.array34[i2 * 4 + i3] = endianConv(br.ReadDouble());
                        }
                    }
    		        for(int i2=0;i2<4;i2++){
                        new_param.dist_factor[i2] = endianConv(br.ReadDouble());
    		        }
    		        result[i]=new_param;
                }
                return result;
