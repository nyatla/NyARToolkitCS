...
public class NyARDetectMarker
{
	...
	private class RleDetector : NyARSquareContourDetector_Rle
	{
		...
		private sealed NyARMatchPattResult __detectMarkerLite_mr=new NyARMatchPattResult();
		...
		public RleDetector(INyARColorPatt i_inst_patt,NyARCode[] i_ref_code,int i_num_of_code,NyARParam i_param)
		{
			...
			sealed int cw = i_ref_code[0].getWidth();
			sealed int ch = i_ref_code[0].getHeight();
		...
		protected void onSquareDetect(NyARIntCoordinates i_coord,int[] i_vertex_index)
		{
			...
			sealed NyARDetectMarkerResult result = this.result_stack.prePush();
			result.arcode_id = square_index;
			result.confidence = confidence;

			sealed NyARSquare sq=result.square;
	...
	private static sealed int AR_SQUARE_MAX = 300;
		...
		int			i_number_of_code)
	{
		...
		sealed NyARIntSize scr_size=i_ref_param.getScreenSize();
		// 解析オブジェクトを作る
		sealed int cw = i_ref_code[0].getWidth();
		sealed int ch = i_ref_code[0].getHeight();
	...
	{
		sealed NyARDetectMarkerResult result = this._square_detect.result_stack.getItem(i_index);
