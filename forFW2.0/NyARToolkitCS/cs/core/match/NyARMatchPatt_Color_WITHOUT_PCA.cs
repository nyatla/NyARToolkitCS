...
public class NyARMatchPatt_Color_WITHOUT_PCA : INyARMatchPatt
{
	...
	public bool evaluate(NyARMatchPattDeviationColorData i_patt,NyARMatchPattResult o_result)
	{
		...
		//
		sealed int[] linput = i_patt.getData();
		int sum;
		double max = Double.MIN_VALUE;
		int res = NyARMatchPattResult.DIRECTION_UNKNOWN;
		sealed int for_mod=this._optimize_for_mod;
		for (int j = 0; j < 4; j++) {
			//合計値初期化
			sum=0;
			sealed NyARMatchPattDeviationColorData code_patt=this._code_patt.getColorData(j);
			sealed int[] pat_j = code_patt.getData();
			...
			//<全画素について、比較(FORの1/16展開)/>
			sealed double sum2 = sum / code_patt.getPow();// sum2 = sum / patpow[k][j]/ datapow;
			if (sum2 > max) {
				max = sum2;
				res = j;
			}
