...
public class NyARMatchPattDeviationBlackWhiteData
{
	...
	public void setRaster(INyARRaster i_raster)
	{
		...
		int ave;//<PV/>
		int rgb;//<PV/>
		sealed int[] linput=this._data;//<PV/>
		sealed int[] buf=(int[])i_raster.getBuffer();

		// input配列のサイズとwhも更新// input=new int[height][width][3];
		sealed int number_of_pixels=this._number_of_pixels;
		...
		sealed double p=Math.sqrt((double) sum);
