...
public class NyARColorPatt_PseudoAffine : INyARColorPatt
{
	...
	private static sealed int BUFFER_FORMAT=NyARBufferType.INT1D_X8R8G8B8_32;
	/**
	 * この関数はラスタの幅を返します。
	 */		
	 ...
	 */	
	public sealed NyARIntSize getSize()
	{
		return 	this._size;
	}
	/**
	 ...
	 */	
	public sealed INyARRgbPixelDriver getRgbPixelDriver()
	{
	 ...
	 */	
	public sealed int getBufferType()
	{
	 ...
	 */	
	public sealed bool isEqualBufferType(int i_type_value)
	{
	...
	{		
		...
		sealed NyARDoubleMatrix44 mat=this._invmat;
		mat.m00=0;
		mat.m01=0;
		mat.m02=0;
		mat.m03=1.0;
	...
	{
		sealed NyARDoubleMatrix44 invmat=this._invmat;
		double v1,v2,v4;
	...
	{
		sealed double[] conv_param=this._convparam;
	    int rx2,ry2;
		rx2=this._size.w;
		ry2=this._size.h;
		...
		for(int y=0;y<ry2;y++){
			for(int x=0;x<rx2;x++){
				sealed int ttx=(int)((conv_param[0]*x*y+conv_param[1]*x+conv_param[2]*y+conv_param[3])+0.5);
				sealed int tty=(int)((conv_param[4]*x*y+conv_param[5]*x+conv_param[6]*y+conv_param[7])+0.5);
