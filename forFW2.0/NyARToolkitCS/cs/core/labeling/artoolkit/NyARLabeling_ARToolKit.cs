...
{
	private static sealed int WORK_SIZE = 1024 * 32;// #define WORK_SIZE 1024*32

	private sealed NyARWorkHolder _work_holder = new NyARWorkHolder(WORK_SIZE);
	...
	public int labeling(NyARBinRaster i_raster,NyARLabelingImage o_destination)
	{
		...
		sealed int lxsize = in_size.w;// lxsize = arUtil_c.arImXsize;
		sealed int lysize = in_size.h;// lysize = arUtil_c.arImYsize;
		sealed int[] label_img = (int[]) o_destination.getBuffer();
...
{
	private sealed static int ARRAY_APPEND_STEP = 256;

	public sealed int[][] work2;
	 ...
	 * @param i_index
	 */
	public sealed void reserv(int i_index)
	{
		// アロケート済みなら即リターン
