...
public abstract class NyARLabeling_Rle
{
	 ...
	 * @throws NyARException
	 */
	private sealed bool addFragment(RleElement i_rel_img, int i_nof, int i_row_index,RleInfoStack o_stack)
	{
		int l=i_rel_img.l;
		sealed int len=i_rel_img.r - l;
	...
	private void imple_labeling(INyARRaster i_raster,int i_th,int i_left,int i_top,int i_width, int i_height)
	{
		...
		// リセット処理
		sealed RleInfoStack rlestack=this._rlestack;
		rlestack.clear();

		//
		int len_prev = 0;
		int len_current = 0;
		sealed int bottom=i_top+i_height;
		int id_max = 0;
		int label_count=0;
		int ypos=i_top;
		// 初段登録
		...
		for (int y = i_top + 1; y < bottom; y++) {
			...
			SCAN_CUR: for (int i = 0; i < len_current; i++) {
				...
				SCAN_PREV: while (index_prev < len_prev) {
					...
					//
					sealed int l= rle_current[i].l;
					sealed int r= rle_current[i].r;
					sealed int len=r-l;
					//結合先フラグメントの情報を更新する。
					id_ptr.area += len;
					...
					while (index_prev < len_prev) {
						...
						//結合するルートフラグメントを取得
						sealed int prev_id =rle_prev[index_prev].fid;
		...
		//対象のラベルだけを追記
		sealed int max=this._max_area;
		sealed int min=this._min_area;
		for(int i=id_max-1;i>=0;i--){
			sealed NyARRleLabelFragmentInfo src_info=f_array[i];
			sealed int area=src_info.area;
	 ...
	 * このクラスでは、true固定です。
	 */
	public sealed static bool _sf_label_array_safe_reference=true;
}
...
class NyARRlePixelDriver_BIN_GS8 : NyARLabeling_Rle.IRasterDriver
{
	...
	public int xLineToRle(int i_x,int i_y,int i_len,int i_th,NyARLabeling_Rle.RleElement[] i_out)
	{
		...
		int x = st;
		sealed int right_edge = st + i_len - 1;
		while (x < right_edge) {
			// 暗点(0)スキャン
			if (buf[x] > i_th) {
				x++;//明点
