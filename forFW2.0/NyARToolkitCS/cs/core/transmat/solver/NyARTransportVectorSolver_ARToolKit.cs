...
{
	private sealed NyARMat _mat_at = new NyARMat(3,8);//3,NUMBER_OF_VERTEX*2
	private sealed NyARMat _mat_a =  new NyARMat(8,3);//NUMBER_OF_VERTEX,3
	private sealed NyARMat _mat_t =  new NyARMat(3,3);//NUMBER_OF_VERTEX,3
	private sealed NyARMat _mat_c =  new NyARMat(8,1);//NUMBER_OF_VERTEX * 2, 1
	private sealed NyARMat _mat_e =  new NyARMat(3,1);
	private sealed NyARMat _mat_f =  new NyARMat(3,1);
	private double[] _cx=new double[4];
	private double[] _cy=new double[4];
	
	private sealed NyARPerspectiveProjectionMatrix _projection_mat;
	/**
	 * コンストラクタです。
	...
	public NyARTransportVectorSolver_ARToolKit(NyARPerspectiveProjectionMatrix i_projection_mat_ref)
	{
		...
		//aとb(aの転置行列)の固定部分を設定。
		sealed double[][] mata = this._mat_a.getArray();
		sealed double[][] matat = this._mat_at.getArray();
	...
	{		
		...
		sealed double[] cx=this._cx;
		sealed double[] cy=this._cy;
		sealed double cpara02=this._projection_mat.m02;
		sealed double cpara12=this._projection_mat.m12;		
		sealed NyARMat mat_t=this._mat_t;
		sealed double[][] mata = this._mat_a.getArray();
		sealed double[][] matat= this._mat_at.getArray();
		for (int i = 0; i < 4; i++){
	...
	{
		sealed double[][] matc = this._mat_c.getArray();
		sealed double cpara00=this._projection_mat.m00;
		sealed double cpara01=this._projection_mat.m01;
		sealed double cpara02=this._projection_mat.m02;
		sealed double cpara11=this._projection_mat.m11;
		sealed double cpara12=this._projection_mat.m12;
		sealed double[] cx=this._cx;
		sealed double[] cy=this._cy;
		...
		for (int i = 0; i < 4; i++) {
			sealed int x2 = i+i;
			sealed NyARDoublePoint3d point3d_ptr=i_vertex3d[i];
			//透視変換？
		...
		sealed double[][] matf = this._mat_f.getArray();
