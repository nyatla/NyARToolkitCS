...
public class NyARRotMatrix : NyARDoubleMatrix33
{
	...
	sealed private NyARRotVectorV2 __initRot_vec1;
	sealed private NyARRotVectorV2 __initRot_vec2;
	/**
	 * この関数は、{@link NyARTransMatResult}の内容を、回転行列にセットします。
	 * @param i_prev_result
	 * セットする姿勢変換行列。
	 */
	public sealed void initRotByPrevResult(NyARTransMatResult i_prev_result)
	{
	 ...
	 * @throws NyARException
	 */
	public void initRotBySquare(sealed NyARLinear[] i_linear,sealed NyARDoublePoint2d[] i_sqvertex)
	{
		sealed NyARRotVectorV2 vec1=this.__initRot_vec1;
		sealed NyARRotVectorV2 vec2=this.__initRot_vec2;
		...
		//最後の軸を計算
		sealed double w02 = vec1.v2 * vec2.v3 - vec1.v3 * vec2.v2;
		sealed double w12 = vec1.v3 * vec2.v1 - vec1.v1 * vec2.v3;
		sealed double w22 = vec1.v1 * vec2.v2 - vec1.v2 * vec2.v1;
		sealed double w = Math.sqrt(w02 * w02 + w12 * w12 + w22 * w22);
		this.m02 = w02/w;
		this.m12 = w12/w;
		this.m22 = w22/w;
		return;
	 ...
	 * ZXY系のラジアン値
	 */
	public sealed void initRotByAngle(NyARDoublePoint3d i_angle)
	{
	 ...
	 * 回転した三次元座標
	 */
	public sealed void getPoint3d(sealed NyARDoublePoint3d i_in_point,sealed NyARDoublePoint3d i_out_point)
	{
		sealed double x=i_in_point.x;
		sealed double y=i_in_point.y;
		sealed double z=i_in_point.z;
	 ...
	 * 回転する座標の個数
	 */
	public sealed void getPoint3dBatch(sealed NyARDoublePoint3d[] i_in_point,NyARDoublePoint3d[] i_out_point,int i_number_of_vertex)
	{
		for(int i=i_number_of_vertex-1;i>=0;i--){
			sealed NyARDoublePoint3d out_ptr=i_out_point[i];
			sealed NyARDoublePoint3d in_ptr=i_in_point[i];
			sealed double x=in_ptr.x;
			sealed double y=in_ptr.y;
			sealed double z=in_ptr.z;
