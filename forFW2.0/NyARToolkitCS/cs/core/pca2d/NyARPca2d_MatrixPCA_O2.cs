...
{
	private static sealed double PCA_EPS = 1e-6; // #define EPS 1e-6

	private static sealed int PCA_MAX_ITER = 100; // #define MAX_ITER 100

	private static sealed double PCA_VZERO = 1e-16; // #define VZERO 1e-16
	...
	private void PCA_PCA(double[] i_v1,double[] i_v2,int i_number_of_data,NyARDoubleMatrix22 o_matrix, double[] o_ev,double[] o_mean)
	{
		...
		sealed double srow = Math.sqrt((double) i_number_of_data);
		double w00, w11, w10;
		w00 = w11 = w10 = 0.0;// *out = 0.0;
		for (int i = 0; i < i_number_of_data; i++) {
			sealed double x = (i_v1[i] - sx) / srow;
			sealed double y = (i_v2[i] - sy) / srow;
			w00 += (x * x);// *out += *in1 * *in2;
			w10 += (x * y);// *out += *in1 * *in2;
			w11 += (y * y);// *out += *in1 * *in2;
		}
	...
	public void pca(double[] i_v1,double[] i_v2,int i_number_of_point,NyARDoubleMatrix22 o_evec, double[] o_ev,double[] o_mean)
	{
		...
		sealed double sum = o_ev[0] + o_ev[1];
		// For順変更禁止
