...
namespace jp.nyatla.nyartoolkit.cs.sandbox.x2
{
     ...
     *
     */
    public class NyARFixedFloatPca2d : INyARPca2d
    {
         ...
         */
        private static void PCA_QRM(NyARI64Matrix22 o_matrix, NyARI64Point2d dv)
        {
            long abs_x;
            long w, t, s, x, y, c;
            long ev1;
         ...
         * @throws NyARException
         */
        private void PCA_PCA(int[] i_x, int[] i_y, int i_number_of_data, NyARI64Matrix22 o_matrix, NyARI64Point2d o_ev, NyARI64Point2d o_mean)
        {
            ...
            long srow = NyMath.sqrtFixdFloat16((long)i_number_of_data << 16);//S16
            long w00, w11, w10;
            w00 = w11 = w10 = 0;// *out = 0.0;
            for (int i = 0; i < i_number_of_data; i++)
            {
                long x = ((i_x[i] - sx) << 16) / srow;//S6
                long y = ((i_y[i] - sy) << 16) / srow;//S6
                w00 += (x * x) >> 16;//S16
                w10 += (x * y) >> 16;//S16
                w11 += (y * y) >> 16;//S16
            }
        ...
        public void pca(double[] i_x, double[] i_y, int i_number_of_point, NyARDoubleMatrix22 o_evec, NyARDoublePoint2d o_ev, NyARDoublePoint2d o_mean)
        {
            ...
            //値のセット
            int[] x_ptr = this.__pca_tmpx;
            int[] y_ptr = this.__pca_tmpy;
         ...
         * @throws NyARException
         */
        public void pcaF16(int[] i_x, int[] i_y, int i_number_of_point, NyARI64Matrix22 o_evec, NyARI64Point2d o_ev, NyARI64Point2d o_mean)
        {
            //計算
            PCA_PCA(i_x, i_y, i_number_of_point, o_evec, o_ev, o_mean);
            long sum = o_ev.x + o_ev.y;
            // For順変更禁止
