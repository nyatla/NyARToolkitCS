...
namespace jp.nyatla.nyartoolkit.cs.detector
{
    ...
    public class NyARDetectMarker{
         ...
         * @throws NyARException
         */
        public int detectMarkerLite(NyARRaster i_image,int i_thresh)
        {
	        ...
	        {
	            NyARSquare csquare=l_square_list.getSquare(i);
	            //評価基準になるパターンをイメージから切り出す
                if (!this.patt.pickFromRaster(i_image, csquare))
                {
    	            //イメージの切り出しは失敗することもある。
    	            continue;
                }
