...
namespace jp.nyatla.nyartoolkit.cs.core
{
    public interface NyARLabeling{
         ...
         * @throws NyARException
         */
        void labeling(NyARRaster image,int thresh);
    }
    ...
    public class NyARLabelHolder
    {
        private const int ARRAY_APPEND_STEP=128;
        public NyARLabel[] labels;
        private int allocate_size;
        /**
         * 最大i_holder_size個の動的割り当てバッファを準備する。
         * @param i_holder_size
         ...
         * @param i_index
         */
        private void reserv(int i_index)
        {
	    //アロケート済みなら即リターン
	    if(this.allocate_size>i_index){
	        return;
         ...
         * @throws NyARException
         */
        public void init(int i_reserv_size,int i_lxsize,int i_lysize)
        {
     ...
     *
     */
    public class NyARLabeling_O2 : NyARLabeling
    {
        private const int WORK_SIZE=1024*32;//#define WORK_SIZE   1024*32
        private int[,] glabel_img;//static ARInt16 l_imageL[HARDCODED_BUFFER_WIDTH*HARDCODED_BUFFER_HEIGHT];

        private NyARWorkHolder work_holder=new NyARWorkHolder(WORK_SIZE);
        private NyARLabelHolder label_holder=new NyARLabelHolder(WORK_SIZE);
         ...
         * @throws NyARException
         */
        public void labeling(NyARRaster image,int thresh)
        {
	        int wk_max;                   /*  work                */
	        int m,n;                      /*  work                */
	        int lxsize, lysize;
...
//                label_img_pt1=label_img[j-1];
		        ...
		        if(line_bufferr[i]<=thresht3){
                    ...
                    if (label_img[j - 1,i] > 0)
                    {//if( *pnt1 > 0 ) {
                        label_pixel = label_img[j - 1,i];//*pnt2 = *pnt1;
                    ...
                    else if (label_img[j - 1, i + 1] > 0)
                    {//}else if( *(pnt1+1) > 0 ) {
                        if (label_img[j - 1, i - 1] > 0)
                        {//if( *(pnt1-1) > 0 ) {
                            m = work[label_img[j - 1, i + 1] - 1];//m = work[*(pnt1+1)-1];
                            n = work[label_img[j - 1, i - 1] - 1];//n = work[*(pnt1-1)-1];
			            if( m > n ){
                    ...
                    else if ((label_img[j,i - 1]) > 0)
                    {//}else if( *(pnt2-1) > 0 ) {
                        m = work[(label_img[j - 1, i + 1]) - 1];//m = work[*(pnt1+1)-1];
                        n = work[label_img[j, i - 1] - 1];//n = work[*(pnt2-1)-1];
			            if( m > n ) {
                        ...
                        label_pixel = label_img[j - 1, i + 1];//*pnt2 = *(pnt1+1);
                    ...
                    {//}else if( *(pnt2-1) > 0) {
                        label_pixel = label_img[j, i - 1];//*pnt2 = *(pnt2-1);
                    ...
                    label_img[j, i] = label_pixel;
		        }else {
                    label_img[j, i] = 0;//*pnt2 = 0;
		        }
