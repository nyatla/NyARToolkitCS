...
namespace jp.nyatla.nyartoolkit.cs.core
{
    ...
    public class BinomialPyramid32f : GaussianScaleSpacePyramid
    {
        readonly private int[] mTemp_us16;
        readonly private double[] mTemp_f32_1;
        public BinomialPyramid32f(int i_width, int i_height, int i_num_of_octaves, int i_num_scales_per_octaves)
            : base(i_num_of_octaves, i_num_scales_per_octaves)
        {
        ...
        private void binomial_4th_order(double[] dst, int[] tmp, int[] src, int width, int height)
        {
            ...
            Debug.Assert(width >= 5);//), "Image is too small");
            Debug.Assert(height >= 5);//, "Image is too small");
