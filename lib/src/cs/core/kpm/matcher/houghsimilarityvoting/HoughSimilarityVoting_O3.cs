...
namespace jp.nyatla.nyartoolkit.cs.core
{
     ...
     * correspondences.
     */
    sealed public class HoughSimilarityVoting_O3
    {
        private const double PI = NyARMath.PI;
        private const double kHoughBinDelta = 1;

        // Min/Max (x,y,scale). The angle includes all angles (-pi,pi).
        readonly private double mMinX;
        readonly private double mMaxX;
        readonly private double mMinY;
        readonly private double mMaxY;
        readonly private double mMinScale;
        readonly private double mMaxScale;
        readonly private double mScaleOneOverLogK;

        private int mNumXBins;
        private int mNumYBins;
        readonly private int mNumAngleBins;
        readonly private int mNumScaleBins;
        ...
        private class SubBinLocation : BinLocation
        {
            public int index;
        ...
        private class TreeVoteMap
        {
            //public int nol=0;//debug
            const int NUM_OF_ARRAY = 16;

            private NodePool pool;
            private VoteNode root;
            ...
            public bool vote(int x, int y, int angle, int scale, int weight)
            {
                VoteNode r;
            ...
            private class NodePool : NyARObjectStack<VoteNode>
            {
                public NodePool(int i_length)
                    : base(i_length)
                {
                }
                override protected VoteNode createElement()
                {
