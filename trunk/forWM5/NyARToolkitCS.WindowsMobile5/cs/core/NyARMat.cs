            ...
            for(int i = 0; i < clm; i++ ) {
                for(j = 0; j < clm; j++ ) {
                    if( j < i ) {
                        ...
                        w=0.0;//*out = 0.0;
                        for(k = 0; k < row; k++ ){
                            _in=input.m[k];//in=input.getRowArray(k);
                            w+=(_in[i]*_in[j]);//*out += *in1 * *in2;
                        }
                        out_m[i][j]=w;
                    }
                }
