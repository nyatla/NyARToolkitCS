...
{	
	...
	private sealed NyARLabelOverlapChecker<NyARRleLabelFragmentInfo> _overlap_checker = new NyARLabelOverlapChecker<NyARRleLabelFragmentInfo>(32,NyARRleLabelFragmentInfo.class);
	...
	public void detectMarker(INyARGrayscaleRaster i_raster,NyARIntRect i_area,int i_th)
	{
		...
		sealed NyARRleLabelFragmentInfoPtrStack flagment=this._labeling.label_stack;
		sealed NyARLabelOverlapChecker<NyARRleLabelFragmentInfo> overlap = this._overlap_checker;

		// ラベル数が0ならここまで
		this._labeling.labeling(i_raster, i_area, i_th);
		sealed int label_num=flagment.getLength();
		if (label_num < 1) {
			return;
		}
	...
	public void detectMarker(INyARGrayscaleRaster i_raster,int i_th)
	{
		...
		for (int i=0; i < label_num; i++) {
			sealed NyARRleLabelFragmentInfo label_pt=labels[i];
			int label_area = label_pt.area;
