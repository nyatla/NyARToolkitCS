...
public class NyARSystemOfLinearEquationsProcessor
{
	...
	{		
		sealed int sm=i_left.length;
		sealed int sn=i_left[0].length;
		if(i_left.length!=sm){
			return false;
		}
		...
		for(int solve_row=0;solve_row<i_m;solve_row++)
		{
			{//ピボット操作
				...
				for(int i=solve_row+1;i<i_m;i++){
					sealed double pivod_2=Math.abs(i_left[i][pivod]);
				...
				if(solve_row!=pivod){
					...
					sealed double[] t=i_left[solve_row];
					i_left[solve_row]=i_left[pivod];
					i_left[pivod]=t;
					sealed double t2=i_right[solve_row];
					i_right[solve_row]=i_right[pivod];
					i_right[pivod]=t2;
				}
			}
			sealed double[] dest_l_n=i_left[solve_row];
			sealed double dest_l_nn=i_left[solve_row][solve_row];
			if(dest_l_nn==0.0){
			...
			for(int i=0;i<solve_row;i++){
				...
				{
					sealed double p=i_left[i][i2]*s;
					dest_l_n[i2]=dest_l_n[i2]-p;
				}
				sealed double k=i_right[i]*s;
				i_right[solve_row]=i_right[solve_row]-k;
				
			}
			//消去法の実行(割り算)
			sealed double d=dest_l_n[solve_row];
