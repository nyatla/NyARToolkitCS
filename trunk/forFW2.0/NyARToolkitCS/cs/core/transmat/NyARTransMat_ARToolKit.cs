...
{
	private sealed static int AR_GET_TRANS_MAT_MAX_LOOP_COUNT = 5;// #define AR_GET_TRANS_MAT_MAX_LOOP_COUNT 5
	private sealed static double AR_GET_TRANS_MAT_MAX_FIT_ERROR = 1.0;// #define AR_GET_TRANS_MAT_MAX_FIT_ERROR 1.0
	private sealed static double AR_GET_TRANS_CONT_MAT_MAX_FIT_ERROR = 1.0;
	...
	{
		sealed NyARCameraDistortionFactor dist=i_ref_distfactor;
		sealed NyARPerspectiveProjectionMatrix pmat=i_ref_projmat;
	...
	private sealed NyARDoublePoint2d[] __transMat_vertex_2d = NyARDoublePoint2d.createArray(4);
	private sealed NyARDoublePoint3d[] __transMat_vertex_3d = NyARDoublePoint3d.createArray(4);
	private sealed NyARDoublePoint3d __transMat_trans=new NyARDoublePoint3d();
	/**
	 * この関数は、理想座標系の四角系を元に、位置姿勢変換行列を求めます。
	 * ARToolKitのarGetTransMatに該当します。
	 * @see INyARTransMat#transMatContinue
	 */
	public sealed bool transMat(NyARSquare i_square,NyARRectOffset i_offset, NyARTransMatResult o_result_conv)
	{
		sealed NyARDoublePoint3d trans=this.__transMat_trans;
	 ...
	 * @see INyARTransMat#transMatContinue
	 */
	public sealed bool transMatContinue(NyARSquare i_square,NyARRectOffset i_offset,NyARTransMatResult i_prev_result,NyARTransMatResult o_result)
	{
		sealed NyARDoublePoint3d trans=this.__transMat_trans;
		...
		if (err > AR_GET_TRANS_CONT_MAT_MAX_FIT_ERROR) {
			...
			sealed double err2=this.optimize(this._rotmatrix, trans, this._transsolver,i_offset.vertex, vertex_2d);
			//エラー値が低かったら値を差換え
			if (err2 < err) {
				// 良い値が取れたら、差換え
