...
{
	private sealed NyARPerspectiveProjectionMatrix _projection_mat_ref;
	/**
	 * コンストラクタです。
	...
	private sealed double[][] __modifyMatrix_double1D = new double[8][3];
	/**
	 * この関数は、回転行列を最適化します。
	...
	public double modifyMatrix(NyARRotMatrix_ARToolKit io_rot,NyARDoublePoint3d trans, NyARDoublePoint3d[] i_vertex3d, NyARDoublePoint2d[] i_vertex2d)
	{
		...
		// vertex展開
		sealed double VX00, VX01, VX02, VX10, VX11, VX12, VX20, VX21, VX22, VX30, VX31, VX32;
		VX00 = i_vertex3d[0].x;
		VX01 = i_vertex3d[0].y;
		VX02 = i_vertex3d[0].z;
		VX10 = i_vertex3d[1].x;
		...
		VX31 = i_vertex3d[3].y;
		VX32 = i_vertex3d[3].z;
		sealed double P2D00, P2D01, P2D10, P2D11, P2D20, P2D21, P2D30, P2D31;
		...
		sealed NyARPerspectiveProjectionMatrix prjmat = this._projection_mat_ref;
		sealed double CP0 = prjmat.m00,CP1 = prjmat.m01,CP2 = prjmat.m02,CP4 = prjmat.m10,CP5 = prjmat.m11,CP6 = prjmat.m12,CP8 = prjmat.m20,CP9 = prjmat.m21,CP10 = prjmat.m22;
		...
		sealed double[][] double1D = this.__modifyMatrix_double1D;

		sealed double[] a_factor = double1D[1];
		sealed double[] sinb = double1D[2];
		sealed double[] cosb = double1D[3];
		sealed double[] b_factor = double1D[4];
		sealed double[] sinc = double1D[5];
		sealed double[] cosc = double1D[6];
		sealed double[] c_factor = double1D[7];
		double w, w2;
		double wsin, wcos;
		
		sealed NyARDoublePoint3d angle = io_rot.getAngle();
		a2 = angle.x;
		b2 = angle.y;
		c2 = angle.z;
