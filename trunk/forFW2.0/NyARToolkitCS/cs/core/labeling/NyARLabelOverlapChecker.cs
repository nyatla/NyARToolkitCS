 ...
 */
public class NyARLabelOverlapChecker<T : NyARLabelInfo>
{
	private T[] _labels;
	private int _length;
	...
	{
		// 重なり処理かな？
		sealed T[] label_pt = this._labels;
		sealed int px1 = (int) i_label.pos_x;
		sealed int py1 = (int) i_label.pos_y;
		for (int i = this._length - 1; i >= 0; i--) {
			sealed int px2 = (int) label_pt[i].pos_x;
			sealed int py2 = (int) label_pt[i].pos_y;
			sealed int d = (px1 - px2) * (px1 - px2) + (py1 - py2) * (py1 - py2);
