...
public class NyARMatchPatt_BlackWhite : INyARMatchPatt
{
	...
	public bool evaluate(NyARMatchPattDeviationBlackWhiteData i_patt,NyARMatchPattResult o_result)
	{
		...
		for (int j = 0; j < 4; j++) {
			//合計値初期化
			sum=0;
			sealed NyARMatchPattDeviationBlackWhiteData code_patt=this._ref_code_patt.getBlackWhiteData(j);
			...
			sealed double sum2 = sum / code_patt.getPow() / i_patt.getPow();// sum2 = sum / patpow[k][j]/ datapow;
			if (sum2 > max) {
				max = sum2;
				res = j;
			}
