...
public class NyARColorPatt_O3 : NyARColorPatt_Base
{
	...
	private sealed NyARMat wk_get_cpara_a = new NyARMat(8, 8);
	private sealed NyARMat wk_get_cpara_b = new NyARMat(8, 1);
	private sealed NyARMat wk_pickFromRaster_cpara = new NyARMat(8, 1);
	 ...
	 * @throws NyARException
	 */
	private bool get_cpara(sealed NyARIntPoint2d[] i_vertex, NyARMat o_para)throws NyARException
	{
	 ...
	 */
	public bool pickFromRaster(INyARRgbRaster image,NyARIntPoint2d[] i_vertexs)throws NyARException
	{
	...
	private void updateExtpat(INyARRgbRaster image, NyARMat i_cpara, int i_xdiv2,int i_ydiv2)
	{
		...
		int r,g,b;
		//ピクセルリーダーを取得
		sealed int pat_size_w=this._size.w;
		sealed int xdiv = i_xdiv2 / pat_size_w;// xdiv = xdiv2/Config.AR_PATT_SIZE_X;
		sealed int ydiv = i_ydiv2 / this._size.h;// ydiv = ydiv2/Config.AR_PATT_SIZE_Y;
		sealed int xdiv_x_ydiv = xdiv * ydiv;
		double reciprocal;
		sealed double[][] para=i_cpara.getArray();
		sealed double para00=para[0*3+0][0];
		sealed double para01=para[0*3+1][0];
		sealed double para02=para[0*3+2][0];
		sealed double para10=para[1*3+0][0];
		sealed double para11=para[1*3+1][0];
		sealed double para12=para[1*3+2][0];
		sealed double para20=para[2*3+0][0];
		sealed double para21=para[2*3+1][0];

		INyARRgbPixelDriver reader=image.getRgbPixelDriver();
		sealed int img_width=image.getWidth();
		sealed int img_height=image.getHeight();

		//ワークバッファの準備
		reservWorkBuffers(xdiv,ydiv);
		sealed double[] xw=this.__updateExtpat_xw;
		sealed double[] yw=this.__updateExtpat_yw;
		sealed int[] xc=this.__updateExtpat_xc;
		sealed int[] yc=this.__updateExtpat_yc;
		...
		for(int iy=this._size.h-1;iy>=0;iy--){
			for(int ix=pat_size_w-1;ix>=0;ix--){
				...
				for(i=ydiv-1;i>=0;i--)
				{
					sealed double para01_x_yw_para02=para01 * yw[i] + para02;
					sealed double para11_x_yw_para12=para11 * yw[i] + para12;
					sealed double para12_x_yw_para22=para21 * yw[i]+ 1.0;
					for(j=xdiv-1;j>=0;j--){
							
						sealed double d = para20 * xw[j] + para12_x_yw_para22;
						if (d == 0) {
							throw new NyARException();
						}
						sealed int xcw= (int) ((para00 * xw[j] + para01_x_yw_para02) / d);
						sealed int ycw= (int) ((para10 * xw[j] + para11_x_yw_para12) / d);
